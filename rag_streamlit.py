...
def streamlit_app():
    ...
    if "initialized" not in st.session_state or not st.session_state.initialized:
        ...
        # Generate CSV and initialize facts
        ...
        try:
            ...
            st.success("🔗 ChromaDB collection initialized successfully!")
        except Exception as e:
            st.error(f"Error initializing ChromaDB collection: {str(e)}")
            return  # Stop the app if there's a critical error

    # Check if collection exists before processing queries
    if "collection" not in st.session_state or not st.session_state.collection:
        st.error("The document collection is not initialized. Please restart the application.")
        return
